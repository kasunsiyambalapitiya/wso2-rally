...
`401` without one. Locally `TOKEN_VALIDATOR_ENABLED=false`, so the service
*decodes* organizer claims without verifying the signature — which means you can
mint yourself a token with no Asgardeo tenant at all:
...
> refuses to start with the validator enabled and no endpoint. It does **not**
> refuse the reverse: nothing stops `false` reaching a deployed environment.
